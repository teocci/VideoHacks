## VideoHacks
Welcome to VideoHacks, a dumping ground for Android graphics & media hacks.
...
- It's not part of the Android Open Source Project.  We cannot accept
  contributions to VideoHacks, even if you have an AOSP CLA on file.
...
To some extent, VideoHacks can be treated as a companion to the
...
VideoHacks's activities as examples.  If you want to understand how the code here works, start
by reading that.
...
lifecycle issues.  VideoHacks is a conventional app, and makes an effort to
handle app issues correctly (like not doing lots of work on the UI thread).

Features are added to VideoHacks as the need arises, often in response to
...
The first time VideoHacks starts, two videos are generated (gen-eight-rects, gen-sliders).
...
Some features of VideoHacks may not work on an emulator, notably anything that involves
encoding video.  The problem is that the software AVC encoder doesn't support
`createInputSurface()`, and all of VideoHacks's video encoding features currently make
